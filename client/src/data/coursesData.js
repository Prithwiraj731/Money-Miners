export const coursesData = [
    {
        id: 'basics-of-trading',
        title: 'Basics of Trading',
        price: 7000,
        duration: '6 Weeks',
        level: 'Beginner',
        thumbnail: '/basic_of_trading.webp',
        shortDesc: 'Master the fundamentals of trading from scratch',
        description: 'Learn the essential skills needed to start your trading journey. This comprehensive course covers market fundamentals, chart reading, and risk management strategies.',
        modules: [
            {
                title: 'Introduction to Financial Markets',
                lessons: [
                    'Understanding Stock Markets',
                    'Types of Financial Instruments',
                    'Market Participants and Their Roles',
                    'Trading Sessions and Market Hours'
                ]
            },
            {
                title: 'Technical Analysis Fundamentals',
                lessons: [
                    'Reading Candlestick Charts',
                    'Support and Resistance Levels',
                    'Trend Lines and Chart Patterns',
                    'Popular Technical Indicators (MA, RSI, MACD)'
                ]
            },
            {
                title: 'Risk Management',
                lessons: [
                    'Position Sizing Strategies',
                    'Stop Loss and Take Profit',
                    'Risk-Reward Ratio Calculation',
                    'Portfolio Diversification Basics'
                ]
            },
            {
                title: 'Developing Your Trading Plan',
                lessons: [
                    'Setting Trading Goals',
                    'Creating a Trading Journal',
                    'Backtesting Strategies',
                    'Psychology of Trading'
                ]
            }
        ],
        whatYouLearn: [
            'Read and interpret price charts confidently',
            'Identify high-probability trading setups',
            'Manage risk effectively to protect capital',
            'Develop a personalized trading strategy'
        ]
    },
    {
        id: 'btc-master',
        title: 'BTC Master',
        price: 8000,
        duration: '8 Weeks',
        level: 'Intermediate',
        thumbnail: '/btc_master.webp',
        shortDesc: 'Become a Bitcoin trading expert',
        description: 'Deep dive into Bitcoin trading strategies, blockchain fundamentals, and advanced technical analysis specific to cryptocurrency markets.',
        modules: [
            {
                title: 'Bitcoin Fundamentals',
                lessons: [
                    'History and Evolution of Bitcoin',
                    'Blockchain Technology Explained',
                    'Bitcoin Halving and Supply Economics',
                    'On-Chain Analysis Basics'
                ]
            },
            {
                title: 'Bitcoin Market Dynamics',
                lessons: [
                    'Crypto Exchanges and Liquidity',
                    'Order Types and Market Microstructure',
                    'Volatility Patterns in Crypto',
                    'Correlation with Traditional Markets'
                ]
            },
            {
                title: 'Advanced Technical Analysis for BTC',
                lessons: [
                    'Fibonacci Retracements in Crypto',
                    'Elliott Wave Theory Application',
                    'Volume Profile and Market Profile',
                    'Rainbow Charts and Logarithmic Analysis'
                ]
            },
            {
                title: 'Bitcoin Trading Strategies',
                lessons: [
                    'Swing Trading Bitcoin',
                    'Scalping Strategies for BTC',
                    'HODLing vs Active Trading',
                    'Leverage Trading and Futures'
                ]
            },
            {
                title: 'Wallet Security & Management',
                lessons: [
                    'Hot vs Cold Wallets',
                    'Hardware Wallet Setup',
                    'Private Key Management',
                    'Tax Implications of Bitcoin Trading'
                ]
            }
        ],
        whatYouLearn: [
            'Understand Bitcoin market cycles and patterns',
            'Execute profitable BTC trades with confidence',
            'Analyze on-chain data for trading insights',
            'Secure and manage your Bitcoin holdings'
        ]
    },
    {
        id: 'crypto-market',
        title: 'Crypto Market (Basic to Advance)',
        price: 8000,
        duration: '10 Weeks',
        level: 'All Levels',
        thumbnail: '/crypto_market.webp',
        shortDesc: 'Complete cryptocurrency trading mastery',
        description: 'From fundamental concepts to advanced DeFi strategies, this comprehensive course covers the entire cryptocurrency ecosystem and trading methodologies.',
        modules: [
            {
                title: 'Cryptocurrency Ecosystem Overview',
                lessons: [
                    'Bitcoin, Ethereum, and Major Altcoins',
                    'Understanding Token Economics',
                    'Layer 1 vs Layer 2 Solutions',
                    'Consensus Mechanisms (PoW, PoS, DPoS)'
                ]
            },
            {
                title: 'Altcoin Trading Strategies',
                lessons: [
                    'Identifying Promising Altcoins',
                    'Market Cap Analysis',
                    'Trading Altcoin Seasons',
                    'Pump and Dump Recognition'
                ]
            },
            {
                title: 'DeFi and Yield Farming',
                lessons: [
                    'Decentralized Exchanges (DEX)',
                    'Liquidity Pools and AMMs',
                    'Yield Farming Strategies',
                    'Impermanent Loss Management'
                ]
            },
            {
                title: 'NFTs and Emerging Trends',
                lessons: [
                    'NFT Market Dynamics',
                    'Layer 2 Scaling Solutions',
                    'Web3 Gaming and Metaverse',
                    'AI Tokens and Future Trends'
                ]
            },
            {
                title: 'Advanced Portfolio Management',
                lessons: [
                    'Portfolio Rebalancing Strategies',
                    'Dollar-Cost Averaging (DCA)',
                    'Tax-Loss Harvesting',
                    'Multi-Chain Portfolio Tracking'
                ]
            },
            {
                title: 'Risk Management in Crypto',
                lessons: [
                    'Hedging with Stablecoins',
                    'Options and Derivatives Trading',
                    'Exchange Risk Management',
                    'Regulatory Compliance'
                ]
            }
        ],
        whatYouLearn: [
            'Trade across multiple cryptocurrency markets',
            'Participate in DeFi ecosystems profitably',
            'Build and manage a diversified crypto portfolio',
            'Navigate emerging trends and opportunities'
        ]
    },
    {
        id: 'gold-digger',
        title: 'Gold Digger Strategy',
        price: 10000,
        duration: '12 Weeks',
        level: 'Advanced',
        thumbnail: '/gold_digger.webp',
        shortDesc: 'Elite momentum trading strategies',
        description: 'Premium course teaching institutional-grade momentum trading strategies, high-frequency techniques, and advanced market microstructure analysis.',
        modules: [
            {
                title: 'Institutional Trading Mindset',
                lessons: [
                    'How Institutions Move Markets',
                    'Order Flow and Smart Money Concepts',
                    'Dark Pools and Hidden Liquidity',
                    'Market Making vs Market Taking'
                ]
            },
            {
                title: 'Advanced Momentum Strategies',
                lessons: [
                    'Identifying Strong Momentum Setups',
                    'Breakout Trading with Confirmation',
                    'Gap Trading Strategies',
                    'News-Based Momentum Plays'
                ]
            },
            {
                title: 'High-Frequency Trading Techniques',
                lessons: [
                    'Scalping with Precision',
                    'Level 2 Order Book Analysis',
                    'Time and Sales Interpretation',
                    'Latency and Execution Optimization'
                ]
            },
            {
                title: 'Market Microstructure',
                lessons: [
                    'Bid-Ask Spread Dynamics',
                    'Liquidity Provision Strategies',
                    'Market Impact and Slippage',
                    'Algorithmic Trading Basics'
                ]
            },
            {
                title: 'Options and Derivatives for Momentum',
                lessons: [
                    'Using Options for Leverage',
                    'Gamma Scalping Strategies',
                    'Volatility Trading',
                    'Futures Spread Trading'
                ]
            },
            {
                title: 'Risk Management for Aggressive Trading',
                lessons: [
                    'Managing Large Position Sizes',
                    'Drawdown Recovery Strategies',
                    'Correlation and Hedging',
                    'Performance Analytics and Optimization'
                ]
            },
            {
                title: 'Building Trading Systems',
                lessons: [
                    'Automating Your Strategy',
                    'Backtesting with Real Data',
                    'Live Trading Execution',
                    'Continuous Improvement Loop'
                ]
            }
        ],
        whatYouLearn: [
            'Execute institutional-grade momentum trades',
            'Understand market microstructure deeply',
            'Scale trading strategies for larger capital',
            'Build and automate profitable trading systems'
        ]
    }
];
